...
class Munkres {
 ...
 std::cout << "Munkres input: " << m << std::endl;
                ...
                if (rows != columns) {
                        ...
                        // and fill the empty values with the largest value
                        // present
                        // in the matrix.
                ...
                // If there were any infinities, replace them with a value
                // greater
 ...
 std::cout << "Munkres output: " << matrix << std::endl;
        ...
        static void minimize_along_direction(Matrix<Data> &matrix,
                ...
                const size_t outer_size = over_columns ? matrix.columns()
                                                       : matrix.rows(),
                             inner_size = over_columns ? matrix.rows()
                                                       : matrix.columns();
                ...
                for (size_t i = 0; i < outer_size; i++) {
                        ...
                        // Start at one because we already have the 0th value in
                        // min.
                        for (size_t j = 1; j < inner_size && min > 0; j++) {
                                min = std::min<double>(min, over_columns
                                                                ? matrix(j, i)
                                                                : matrix(i, j));
                        }
       ...
       private:
        ...
        int step1() {
                ...
                for (size_t row = 0; row < rows; row++) {
                        for (size_t col = 0; col < columns; col++) {
                                if (0 == matrix(row, col)) {
                                        for (size_t nrow = 0; nrow < row;
                                             nrow++)
                                                if (STAR ==
                                                    mask_matrix(nrow, col))
                                                        goto next_column;
        ...
        int step3() {
                ...
                Main Zero Search
                 ...
                 1. Find an uncovered Z in the distance matrix and prime it. If
                no such zero exists, go to Step 5
                 2. If No Z* exists in the row of the Z', go to Step 4.
                 3. If a Z* exists, cover this row and uncover the column of the
                Z*. Return to Step 3.1 to find a new Z
                */
                ...
                for (size_t ncol = 0; ncol < matrix.columns(); ncol++) {
                        if (mask_matrix(saverow, ncol) == STAR) {
                                ...
                                col_mask[ncol] = false;    // uncover the column
                                                           // containing the
                                                           // starred zero
                                return 3;                  // repeat
                        }
                ...
                return 4;  // no starred zero in the row containing this primed
                           // zero
        }
        ...
        int step4() {
                ...
                Increment Set of Starred Zeros
                 ...
                 1. Construct the ``alternating sequence'' of primed and starred
                zeros:

                       Z0 : Unpaired Z' from Step 4.2
                       Z1 : The Z* in the column of Z0
                       Z[2N] : The Z' in the row of Z[2N-1], if such a zero
                exists
                       Z[2N+1] : The Z* in the column of Z[2N]

                    The sequence eventually terminates with an unpaired Z' =
                Z[2N] for some N.
                */
                bool madepair;
                do {
                ...
                for (std::list<std::pair<size_t, size_t> >::iterator i =
                         seq.begin();
                     i != seq.end(); i++) {
                ...
                for (size_t row = 0; row < mask_matrix.rows(); row++) {
                        for (size_t col = 0; col < mask_matrix.columns();
                             col++) {
                                if (mask_matrix(row, col) == PRIME) {
        ...
        int step5() {
                ...
                New Zero Manufactures
                 ...
                 1. Let h be the smallest uncovered entry in the (modified)
                distance matrix.
